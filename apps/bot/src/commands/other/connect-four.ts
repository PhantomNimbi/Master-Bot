...
@ApplyOptions<CommandOptions>({
	name: 'connect-four',
	description: 'Play a game of Connect Four with another member',
	preconditions: ['isCommandDisabled', 'GuildOnly']
})
export class ConnectFourCommand extends Command {
		...
		inviteCollector?.on('end', async (_collected, reason) => {
			...
			if (reason === 'time') {
				...
				if (playerMap.size > 1) {
					...
					return new Connect4Game().connect4(interaction, playerMap);
				}
			}
...
export const help: CommandHelp = {
	...
	category: 'other',
	description: 'Play a game of Connect Four with another member',
	usage: '/connect-four [opponent: @User]',
	examples: ['/connect-four', '/connect-four opponent: @User'],
	options: [
		{
			name: 'opponent',
